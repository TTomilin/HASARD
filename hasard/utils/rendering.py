...
def segment_and_draw_boxes(obs_rgb, state, skip_ids=(0, 1, 255)):
    """
    Convert the original obs (RGB) to a BGR image with each object colored and
    bounding boxes drawn. Displays object names, not just IDs.
    ...
    Returns:
        segmented_bgr (np.ndarray): The resulting BGR image with segmentation
                                    and bounding boxes (object names).
    """
    # 1) Convert RGB->BGR for OpenCV
    obs_bgr = cv2.cvtColor(obs_rgb, cv2.COLOR_RGB2BGR)

    # 2) Create or read label buffer
    ...
    for val in unique_vals:
        ...
        for cnt in contours:
            ...
            # Draw rectangle
            cv2.rectangle(segmented_bgr, (x, y), (x + w, y + h), (0, 255, 0), 2)
            # Put object name instead of ID
            cv2.putText(segmented_bgr, f"{obj_name}", (x, y - 10),
