...
def test_rgb_array_rendering():
    ...
    try:
        ...
        # Create test configuration with higher resolution
        cfg = parse_vizdoom_cfg(["--env=remedy_rush", "--resolution=1280x720"])
        ...
        for i in range(episodes):
            ...
            while not done and step_count < ep_length:  # Limit steps to avoid very long videos
                ...
                done = any(dones) or any(truncated)
                step_count += 1

            print(f"  Episode {i+1} completed: {step_count} steps")
            print(f"  Episode rewards: {[f'{r:.2f}' for r in episode_reward]}")
