...
class PPOPidLearner(PPOLearner):
    def __init__(
            self,
            cfg: Config,
            env_info: EnvInfo,
        ...
        self.pid_lagrangian = PIDLagrangian(kp=cfg.pid_kp, ki=cfg.pid_ki, kd=cfg.pid_kd, target=self.ep_safety_bound,
                                            error_history_length=cfg.pid_d_delay, ema_alpha=cfg.pid_delta_ema_alpha)
        self.lagrangian_multiplier = 0

    def init(self) -> InitModelData:
        init_res = super(PPOPidLearner, self).init()
        return init_res
    ...
    def _prepare_batch(self, batch: TensorDict) -> Tuple[TensorDict, int, int]:
        with torch.no_grad():
            ...
            # Example, ensure this aligns with how you update or compute it elsewhere
            current_lagrange_multiplier = self.lagrangian_multiplier

            # Adjust rewards with the Lagrange multiplier
            adjusted_rewards = buff["rewards"] - current_lagrange_multiplier * buff["costs"]
    ...
    def _calculate_losses(
        ...
        # Adjust rewards by subtracting the PID-adjusted Lagrange multiplier times the cost
        adjusted_rewards = mb.rewards_cpu - self.lagrangian_multiplier * mb.costs_cpu
        ...
        with torch.no_grad(), self.timing.add_time("advantages_returns"):
            if self.cfg.with_vtrace:
                ...
                next_values = values_cpu[recurrence - 1:: recurrence] - adjusted_rewards[recurrence - 1:: recurrence]
                next_values /= self.cfg.gamma
                next_vs = next_values

                for i in reversed(range(self.cfg.recurrence)):
                    rewards = adjusted_rewards[i::recurrence]
    ...
    def _train(
        ...
        for epoch in range(self.cfg.num_epochs):
            ...
            for batch_num in range(len(minibatches)):
                ...
                # current_cost = self.compute_current_cost(mb)
                current_cost = torch.sum(mb.costs)
                self.lagrangian_multiplier = self.pid_lagrangian.update(current_cost)
                mb.rewards = mb.rewards - self.lagrangian_multiplier * mb.costs
    ...
    def _record_summaries(self, train_loop_vars) -> AttrDict:
        ...
        # Additional metrics for PID Lagrangian
        stats.lagrange_multiplier = self.lagrangian_multiplier
        stats.current_cost = torch.sum(var.mb).item()
        stats.pid_error = var.current_cost - self.ep_safety_bound  # Assuming target_cost is accessible
        stats.pid_proportional = self.pid_lagrangian.kp * stats.pid_error
        stats.pid_integral = self.pid_lagrangian.ki * self.pid_lagrangian.integral
        stats.pid_derivative = self.pid_lagrangian.kd * (stats.pid_error - self.pid_lagrangian.previous_error)

        # Optionally log the components of the PID if separated in the PIDLagrangian class
        # stats.pid_p = self.pid_lagrangian.proportional_component
        # stats.pid_i = self.pid_lagrangian.integral_component
        # stats.pid_d = self.pid_lagrangian.derivative_component
