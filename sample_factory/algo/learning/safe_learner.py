...
from sample_factory.model.actor_critic import ActorCritic, create_actor_critic, CostCritic
...
class Learner(Configurable):
    ...
    def init(self) -> InitModelData:
        ...
        # Cost Critic
        # self.cost_critic = CostCritic(global_model_factory(), self.env_info.obs_space, self.env_info.action_space, self.cfg)
        # log.debug("Created Cost Critic model with architecture:")
        # log.debug(self.cost_critic)
        # self.cost_critic.model_to_device(self.device)
        ...
        # self.cost_critic._apply(share_mem)
        # self.cost_critic.train()

        # params = list(self.actor_critic.parameters()) + list(self.cost_critic.parameters())
    ...
    def _calculate_losses(
        ...
        with self.timing.add_time("losses"):
            ...
            # Version from Safety-MARL
            # safety_bound = 0.0  # TODO obtain scenario specific safety bound
            # delta_lambda_lagr = -((mb["costs"].mean() - safety_bound) * (1 - self.cfg.gamma) + (ratio * cost_adv)).mean().detach()
            # new_lambda_lagr = self.lambda_lagr - (delta_lambda_lagr * self.cfg.lagrangian_coef_rate)
